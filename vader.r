...
n <- nrow(datos) # 400 rows
n_entrenamiento <- floor(0.8 * n) # 80% de la muestra (320)
muestra <- sample.int(n = n, size = n_entrenamiento, replace = FALSE) # Sample de vectores
entrenamiento <- datos[muestra, ] # Acá se transforma a dataframe, poniendo datos[vector de sample]
prueba <- datos[-muestra, ] # El resto de la muestra (20%) (80)

columnas <- colnames(datos) # Todas las variables iniciales
print(columnas)
i_es_clon <- which(columnas == "es_clon") # Se selecciona el índice de "es_clon"
columnas <- columnas[-i_es_clon] # Se elimina la variable dicotómica es_clon

# Se seleccionan las 8 variables predictoras de forma aleatoria
variables <- sample(columnas, 8) # Ahora si se seleccionan 8 variables
print(variables)
